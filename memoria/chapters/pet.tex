...
teóricos del \Cref{cap:marco-teorico}— y \emph{ejecutable con el
...
\begin{itemize}[leftmargin=*]
    ...
    \item Las matrices afines coinciden exactamente entre los
    volúmenes pareados, lo cual confirma que la reducción de dosis se
    aplica sobre el mismo \textit{grid} espacial sin re-muestreo
    intermedio: los pares están perfectamente registrados a nivel de
    vóxel.
    \item Se asume que cada fichero corresponde a un paciente
    independiente; el patrón de los \textit{timestamps} en los
    nombres es consistente con un proceso de reconstrucción por lotes
    en el operador, no con \textit{frames} dinámicos de un mismo
    paciente.
\end{itemize}

Las estadísticas de intensidad relevantes para el diseño del
preprocesamiento, calculadas sobre 5 volúmenes de referencia son:
...
\begin{itemize}[leftmargin=*]
    ...
    2{,}6)$, lo cual sugiere que una compresión logarítmica produce
    un rango bien centrado para el modelo.
...
volumen no visto. Cada \textit{slice} axial recortado se
redimensiona a $256 \times 256$ píxeles mediante interpolación
bilineal.
...
ruido (\Cref{subsec:schedules}). Los vóxeles que quedan bastante
...
valor bajo). Los extremos vacíos del FOV se descartan para no
...
Los hiperparámetros de optimización, ajustados a la GPU de 32~GB
empleada (\Cref{sec:pet-entorno-cloud}), son:
...
\begin{description}[leftmargin=*]
    ...
    (RTX 4090, 24~GB) habrían obligado a acumulación de gradiente, y
    las tarjetas de centro de datos (A100/H100) multiplican la
    tarifa horaria sin que un modelo de este tamaño las aproveche.
    Con esta máquina se recupera la precisión mixta y el
    \textit{batch} completo, y el entrenamiento queda totalmente
    desacoplado del ordenador personal.
\end{description}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Cómputo}: la instancia GPU (RTX PRO 4500, 32~GB)
    se factura por horas de pod activo. Detener el pod detiene el
    gasto de cómputo, y la combinación de volumen persistente +
    reanudación desde \textit{checkpoint}
    (\Cref{sec:pet-entorno-cloud}) permite hacerlo sin perder
    estado, de modo que solo se paga por horas de GPU realmente
    útiles.
...
cuatro experimentos de entrenamiento.

\begin{table}[h!]
    \centering
    \begin{tabular}{lccc}
        \toprule
        Experimento & Horas GPU & Tarifa (USD/h) & Coste (USD) \\
        \midrule
        Difusión supervisada (Pipeline A) & \todo{} & \todo{} & \todo{} \\
        Difusión incondicional (Pipeline B) & \todo{} & \todo{} & \todo{} \\
        U-Net de regresión (línea base A) & \todo{} & \todo{} & \todo{} \\
        RED-CNN (línea base B) & \todo{} & \todo{} & \todo{} \\
        \midrule
        Evaluaciones e inferencia & \todo{} & \todo{} & \todo{} \\
        \textit{Network volume} (GB-mes) & \multicolumn{2}{c}{\todo{}} & \todo{} \\
        \midrule
        \textbf{Total} & & & \todo{} \\
        \bottomrule
    \end{tabular}
    \caption{Coste de los experimentos sobre RunPod (GPU NVIDIA RTX
    PRO 4500, 32~GB). Cada fila de entrenamiento corresponde a 100
    épocas sobre los $\sim 1{,}9 \times 10^{5}$ pares de cortes de
    entrenamiento.}
    \label{tab:pet-costes}
\end{table}

\todo{Rellenar la \Cref{tab:pet-costes} con las horas y tarifas
reales de RunPod al cierre de los experimentos, y comentar el coste
total frente a la estimación local inicial (150 h de cómputo
acoplado al portátil).}
