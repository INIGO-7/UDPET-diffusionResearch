...
de la observación de dosis baja $\mathbf{y}$--- y las dos familias de
soluciones basadas en difusión ---condicionamiento supervisado y prior
no condicional con guiado de medición--- se presentaron en el
\Cref{cap:marco-teorico}. Siguiendo la trayectoria del estado del arte
(\Cref{sec:estado-del-arte}), se construyen \textbf{ambos enfoques en
paralelo} y se comparan sobre el mismo conjunto de test, junto a dos
líneas base discriminativas que delimitan la aportación real de la
difusión. Cada decisión de diseño se justifica por la literatura o por
los fundamentos teóricos del \Cref{cap:marco-teorico}.
...
\begin{figure}[h!]
    ...
    (\Cref{eq:asinh-norm}) sobre un volumen real del conjunto
    ($M = 11\,827$ cuentas, percentil 99,5; $k = 10$). En el centro, el
    mapeo $x \mapsto x'$, que lleva el origen a $-1$ y el percentil $M$
    a $+1$; alrededor, la distribución de cuentas del \textit{foreground}
    antes (arriba) y después (lateral) de la transformación, que pasa de
    una cola pesada concentrada cerca de cero a un reparto estable en
    $[-1, +1]$.}
...
cores} de la GPU, con prácticamente el doble de rendimiento que
\texttt{float32} y sin riesgo apreciable de desbordamiento, pues
\texttt{bf16} conserva el rango dinámico de \texttt{float32}. Se habilita
...
El plan inicial de entrenar en local sobre el mismo MacBook Pro M4 Pro
(24~GB, \textit{backend} MPS) del experimento de mariposas se descartó
por coste: forzado a \texttt{float32} y \textit{batch} efectivo 16, cada
\textit{pipeline} de difusión estimaba unas \textbf{200~horas} ---más de
ocho días acoplando el portátil al proceso, con cada suspensión como
riesgo operativo---. Se migró la fase PET a \textbf{RunPod}, un proveedor
de GPU bajo demanda con tarifa proporcional al uso, sobre el que se
construyó el entorno reproducible que se describe a continuación.

\subsubsection{Imagen Docker reproducible}

Todo el software se encapsula en una imagen Docker sobre
\texttt{python:3.13-slim} que instala los \textit{wheels} de PyTorch del
índice \texttt{cu124} ---con las bibliotecas CUDA ya empaquetadas, sin
necesidad de la imagen base de NVIDIA---. La imagen no hornea el código:
un \textit{entrypoint} idempotente lo clona desde GitHub y levanta un
servidor SSH antes de cualquier paso costoso, de modo que cada arranque
parte del repositorio actualizado sin reconstruir nada. Publicada en
Docker Hub, sirve de plantilla reutilizable: cualquier instancia que se
lance la descarga y queda con el entorno listo.

\subsubsection{Distribución del dataset preprocesado vía Hugging Face}

Para no resubir 60~GB a cada instancia, la caché de \textit{slices}
(\Cref{sec:pet-preprocesado}) se empaqueta una sola vez en
\textit{shards} de \texttt{tar} y se publica ---junto al fichero de
particiones \texttt{splits.json}--- en un repositorio privado de Hugging
Face Datasets. El \textit{entrypoint} los descarga con
\texttt{hf\_transfer} (descarga paralela de alto ancho de banda),
reconstruye la caché y verifica su integridad comparando el número de
ficheros \texttt{.pt} reconstruidos (477.690) con el esperado.

\subsubsection{Elección de almacenamiento y de máquina}

RunPod separa cómputo y almacenamiento. Un \textit{network volume}
persistente alberga el repositorio, la caché ($\sim 60$~GB), los
\textit{checkpoints} y los \textit{logs}, y sobrevive al apagado de la
GPU ---facturado por GB-mes, una fracción del coste de cómputo
(\Cref{sec:pet-costes})---, de modo que detener la instancia para no
pagar horas ociosas no implica perder estado ni volver a descargar el
dataset. La máquina de referencia es una \textbf{NVIDIA RTX PRO 4500}
(Blackwell, 32~GB de VRAM): sus 32~GB alojan el \textit{batch} de 32
cortes a $256^2$ con el U-Net de 114~M de parámetros y su copia EMA sin
acumulación de gradiente, y fijan el suelo de VRAM para el que se calibra
toda la configuración. La ejecución real, en cambio, se apoyó en un
\textit{pool} rotatorio de tarjetas (\Cref{subsec:pet-pool-gpu}).

\subsubsection{Operativa de ejecución}

La conexión al pod se realiza por SSH; cada experimento corre dentro de
una sesión \texttt{tmux} que desacopla el proceso de la conexión
---cerrar la terminal o reiniciar el portátil no lo interrumpen--- y,
combinada con la reanudación desde \textit{checkpoint}
(\Cref{subsec:pet-config}), sobrevive también a reinicios del propio pod;
la edición puntual se hace con Cursor y la extensión \textit{Remote--SSH}.
El código está organizado para que esta operativa sea de un solo comando:
un \emph{dispatcher} único
(\texttt{python -m src.main \{preprocess|train|reconstruct|evaluate|preview\}})
enruta cada subcomando, con un flag \texttt{--pipeline} que selecciona
entre los dos \textit{pipelines} y las dos líneas base, y un preset
\texttt{--smoke} ($128^2$, 5 épocas, subconjunto de pacientes) que
verifica el flujo completo en aproximadamente una hora antes de
comprometer un entrenamiento largo. La infraestructura compartida (motor
de entrenamiento, validación, E/S de volúmenes, métricas) vive en módulos
comunes, de modo que los cuatro experimentos difieren únicamente en la
construcción de la entrada del modelo y en su inferencia.
...
\textit{checkpoint} convierte la GPU en un recurso intercambiable:
detener un pod y reanudar en otra tarjeta es transparente para el estado
del experimento. Como RunPod es un mercado bajo demanda con
disponibilidad fluctuante, esto permitió asignar a cada experimento la
tarjeta más conveniente entre las disponibles (\Cref{tab:pet-gpus}).
...
con más memoria no añaden \textit{batch}, pero sí más cómputo y la
posibilidad de \textbf{entrenar los dos \textit{pipelines}
simultáneamente} en instancias independientes, reduciendo a la mitad el
tiempo de pared (\Cref{fig:pet-gpu-util}).

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{img/gpu-utilization.png}
    \caption{Panel de monitorización de RunPod durante el entrenamiento
    simultáneo de los dos \textit{pipelines} de difusión sobre dos
    instancias A100 independientes. Ambos pods operan con la GPU
    prácticamente saturada (93--100\,\% de cómputo) y un uso de VRAM
    holgado, por debajo de los 80~GB de la A100.}
...
Estas tres métricas se calculan sobre el \textbf{espacio normalizado}
(la representación $\operatorname{arcsinh}$ en $[-1, +1]$), no sobre el
espacio de cuentas. El PET tiene un rango dinámico altísimo: en cuentas,
el PSNR colapsa hacia el vóxel más brillante ---mediría casi solo cómo se
reproduce el punto más caliente, insensible al resto--- y los valores
atípicos inflarían el SSIM. Como $\operatorname{arcsinh}$ es monótona e
invertible y se aplica por igual a la reconstrucción y al \textit{ground
truth}, no oculta el error: lo repondera para que cuenten los errores
relativos en todo el rango dinámico en lugar de los absolutos en el pico,
sobre una escala común que hace justas las comparaciones entre
volúmenes.
...
\begin{figure}[H]
    ...
    difusión no condicional (DPS). Escala de intensidad común a todos los
    paneles.}
...
\begin{figure}[H]
    ...
    $\omega = 5$. Escala de intensidad común a todos los paneles.}
...
\textbf{$\approx 168$~USD} sobre 152~horas de GPU, repartidas entre las
cuatro tarjetas del \textit{pool}. Las dos líneas discriminativas
ilustran el desacople entre coste y arquitectura: RED-CNN, con
$\sim 1{,}8$~millones de parámetros, completa sus 100 épocas en menos de
la mitad de horas que la U-Net de regresión ---pese a operar a
resolución completa--- y sobre la tarjeta más barata del conjunto. El
saldo total aprovisionado fue algo mayor, pues absorbió además las
pruebas \texttt{-{}-smoke}, la depuración interactiva, algún
relanzamiento y el tiempo de pod ocioso antes de detener cada instancia.
