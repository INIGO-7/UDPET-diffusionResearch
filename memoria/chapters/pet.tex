...
\begin{figure}[h!]
    ...
    \caption{Distribución de cuentas: la cola pesada del PET de dosis
    baja se comprime a una banda acotada. Normalización per-volumen
    estabilizadora de varianza (\Cref{eq:asinh-norm}) aplicada con los
    parámetros reales de un volumen del conjunto ($M = 11\,827$
    cuentas, percentil 99,5; $k = 10$). La curva central traza el
    mapeo $x \mapsto x'$: el origen $x=0$ se envía a $-1$ y el
    percentil de referencia $x=M$ cae exactamente en $+1$; el recuadro
    ampliado muestra el régimen casi lineal cerca del origen y el codo
    suave en $x=k$, donde la transformación pasa al régimen
    logarítmico. El histograma superior es la distribución real de
    cuentas del \textit{foreground} —fuertemente asimétrica y de cola
    pesada, concentrada cerca de cero—; el histograma lateral es esa
    misma distribución tras la transformación, ahora repartida de
    forma estable a lo largo de la banda acotada $[-1, +1]$ sobre la
    que está ajustado el proceso de difusión. El $0,5\%$ de vóxeles más
    brillantes sobresale ligeramente por encima de $+1$ sin
    \textit{clipping}, lo que mantiene exacta la inversión.}
...
Los hiperparámetros de optimización, ajustados al suelo de 32~GB
de VRAM del conjunto de GPUs empleado —de modo que la misma
configuración corre sin cambios en cualquier tarjeta del
\textit{pool} (\Cref{sec:pet-entorno-cloud})—, son:
