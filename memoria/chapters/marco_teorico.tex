...
literatura sobre la que se apoya \emph{directamente} este trabajo. En
lugar de un inventario exhaustivo, se eligen \textbf{tres trabajos de
referencia} —los que se han analizado en mayor profundidad— porque,
entre los tres, contienen las dos ideas que vertebran los
\textit{pipelines} del \Cref{cap:pet}: el \emph{condicionamiento
supervisado por concatenación de canal} y el \emph{guiado posterior}
en inferencia (\Cref{subsec:difusion-condicional}). A continuación se
mencionan, de forma deliberadamente más breve, dos contribuciones muy
recientes que señalan hacia dónde se mueve la frontera del campo, pero
que quedan fuera del alcance de esta memoria.
...
\begin{figure}[h!]
    ...
    la medición (PET-DDS), que produce una imagen plausible pero
    cuantitativamente débil (PSNR $19{,}35$\,dB; SSIM $0{,}729$); y
    dos reconstrucciones \emph{con} guiado posterior por la medición para dos
    escalas de guiado ($\omega = 0{,}25$ y $\omega = 0{,}5$), que
    recuperan la fidelidad a la observación (PSNR $\approx 30$\,dB;
    SSIM $> 0{,}95$). A la derecha, tenemos la imagen de resonancia empleada
    como guiado anatómico opcional. }
    \label{fig:soa-singh}
\end{figure}
...
\paragraph{Lectura conjunta.} Los tres trabajos de referencia dibujan
una trayectoria nítida y, sobre todo, \emph{dos caminos}. Gong et
al.~\cite{ddpmpet2023} siembra ambos sobre \textit{denoising}
cerebral; DDPET-3D~\cite{ddpet3d} madura la vía del
\textbf{condicionamiento supervisado por canal} a escala clínica 3D; y
Singh et al.~\cite{scorepet2024} madura la vía del \textbf{guiado
posterior} con un formalismo riguroso de \textit{score}/SDE. Esos dos
caminos son exactamente los dos \textit{pipelines} que el
\Cref{cap:pet} construye y compara sobre PET real. La frontera más
reciente —\textit{backbones} de espacio de estados~\cite{mamba2025} y
\textit{transformers} generalistas~\cite{tat2025}— diversifica la
arquitectura de la red, pero deja abierta, y en buena medida
ortogonal, la pregunta que este trabajo estudia: cuál de los dos modos
de inyectar la medición rinde mejor para la reconstrucción de PET de
dosis baja.
