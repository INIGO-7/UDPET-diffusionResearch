...
del Instituto Smithsoniano~\cite{smithsonianbutterflies}. Cada imagen se redimensiona a
...
cuadrada. Sobre el resultado se aplica volteo horizontal
...
\textit{tutorial} oficial de la librería
\texttt{diffusers}~\cite{diffuserstutorial}, con
...
\begin{figure}[h!]
    ...
    al.~\cite{ronneberger2015unet}, concebida para segmentación (salida
    de dos canales de clase). El camino de contracción (izquierda)
    reduce la resolución y aumenta los canales; el de expansión
    (derecha) la restaura; las flechas grises horizontales son las
    conexiones de salto. Las U-Net de difusión conservan esta topología
    en «U» y añaden \textit{embeddings} de paso temporal y bloques de
    atención.}
    \label{fig:unet}
\end{figure}
...
diseño de 2015. Conviene detenerse en estos dos tipos de bloque, pues
son los ladrillos con los que se construye toda la red.

\subsection*{Bloques residuales}

Un \textbf{bloque residual}, introducido por He et
al.~\cite{he2016resnet}, no aprende directamente la transformación
deseada $H(\mathbf{x})$, sino el \emph{residuo} respecto a la entrada,
$F(\mathbf{x}) = H(\mathbf{x}) - \mathbf{x}$, de modo que su salida es
$F(\mathbf{x}) + \mathbf{x}$ (\Cref{fig:bloque-residual}). La suma de
la entrada original —la \emph{conexión de salto} interna del bloque,
distinta de las \textit{skip connections} de la «U»— crea un camino
directo por el que el gradiente fluye sin atenuarse, lo que permite
apilar muchos bloques y entrenar redes profundas sin que el gradiente
se desvanezca. Esta es la razón por la que las U-Net modernas
sustituyen las convoluciones simples de 2015 por pilas de bloques
residuales.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.55\linewidth]{img/bloque_residual.png}
    \caption{Esquema del principio residual: la rama principal aplica
    una secuencia de operaciones a la entrada y su salida se
    \emph{suma} a esta a través de la conexión de salto interna
    (\textit{skip}), produciendo $F(\mathbf{x}) + \mathbf{x}$. La
    figura muestra la formulación original (convoluciones con
    \textit{batch normalization} y ReLU); nuestra U-Net de difusión
    las sustituye por \textit{group normalization} y SiLU, e inyecta
    además el \textit{embedding} del paso temporal $t$ entre las dos
    convoluciones, condicionando el bloque al nivel de ruido.}
    \label{fig:bloque-residual}
\end{figure}

\subsection*{Bloques de atención}

El segundo tipo de bloque incorpora un mecanismo de
\textbf{autoatención} (\textit{self-attention}), el mismo que sustenta
los Transformers~\cite{vaswani2017attention} y que, adaptado a mapas
de características convolucionales, popularizó SAGAN~\cite{zhang2019sagan}.
Mientras que una convolución solo combina información dentro de su
ventana local, la autoatención permite que \emph{cada} posición
espacial se relacione con \emph{todas} las demás, capturando
dependencias de largo alcance —por ejemplo, la simetría global de las
alas de una mariposa o, en el caso PET, la coherencia anatómica entre
regiones distantes—. Como ilustra la \Cref{fig:autoatencion}, a partir
del mapa de características de entrada se obtienen mediante tres
convoluciones $1\times1$ tres proyecciones —consulta ($f$), clave
($g$) y valor ($h$)—; el producto entre consulta y clave, normalizado
con \textit{softmax}, produce el \emph{mapa de atención}, que asigna a
cada posición un peso sobre todas las demás; finalmente la salida es
la suma ponderada de los valores según ese mapa.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\linewidth]{img/mecanismo_atencion.png}
    \caption{Mecanismo de autoatención sobre mapas de características
    convolucionales~\cite{zhang2019sagan}. Tres convoluciones
    $1\times1$ proyectan la entrada en consulta $f(\mathbf{x})$, clave
    $g(\mathbf{x})$ y valor $h(\mathbf{x})$; el producto consulta--clave
    seguido de \textit{softmax} genera el mapa de atención, que pondera
    los valores para producir los mapas de características
    autoatendidos.}
    \label{fig:autoatencion}
\end{figure}

El precio de esta capacidad es que el mapa de atención tiene tamaño
$N \times N$ con $N = H \times W$ posiciones espaciales: su coste es
\emph{cuadrático} en el número de píxeles. Aplicarlo a alta resolución
(p.~ej.\ $256^2 = 65\,536$ posiciones) dispararía el consumo de
memoria y cómputo; por ello la atención se reserva para los niveles
profundos de la «U», donde la resolución ya se ha reducido lo
suficiente.
...
(\texttt{AttnUpBlock2D}), el más profundo de la «U», por el motivo
de coste recién expuesto. El paso temporal $t$ se inyecta en cada
...
calentamiento (\textit{warmup}): la tasa crece linealmente desde 0
hasta $\eta$ durante el \textit{warmup} ---evitando actualizaciones
bruscas con el modelo aún sin entrenar--- y decae después siguiendo una
curva cosenoidal hasta aproximarse a 0 en la última época. El
...
Los modelos de difusión generalizan bien y son poco propensos a
memorizar: cada época presentan la misma imagen con un nivel de ruido
distinto, lo que fuerza a la red a aprender la distribución subyacente
---en este caso, el aspecto de una mariposa--- en lugar de ejemplos
concretos. Predecir el ruido de una imagen es, además, matemáticamente
equivalente a predecir la mariposa limpia a partir de su versión
ruidosa.
...
La \Cref{fig:mariposas-curvas} recoge estas dos magnitudes a lo
largo del entrenamiento. A la izquierda, la pérdida MSE cae con
rapidez en las primeras épocas y se estabiliza después en una
meseta prácticamente plana, ilustrando el fenómeno descrito. A la
derecha, la tasa de aprendizaje muestra el perfil del planificador
cosenoidal con calentamiento de la \Cref{sec:mariposas-config}: una
rampa lineal inicial hasta $\eta = 10^{-4}$ seguida del decaimiento
cosenoidal hasta aproximarse a 0 al final del entrenamiento.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.48\linewidth]{img/loss_butterfly_256.png}\hfill
    \includegraphics[width=0.48\linewidth]{img/lr_butterfly_256.png}
    \caption{Curvas de entrenamiento del modelo a resolución
    $256 \times 256$. Izquierda: pérdida MSE por época, que decrece
    abruptamente al inicio y luego se estabiliza. Derecha: evolución
    de la tasa de aprendizaje bajo el planificador cosenoidal con 500
    pasos de calentamiento (rampa lineal seguida de decaimiento
    cosenoidal).}
    \label{fig:mariposas-curvas}
\end{figure}
