...
\begin{figure}[h!]
    ...
    \emph{suma} a la propia entrada a través de la conexión de salto
    interna (\textit{skip}), produciendo $F(\mathbf{x}) + \mathbf{x}$.
    La figura ilustra el concepto general en su formulación original
    (convoluciones con \textit{batch normalization} y ReLU); en
    nuestra U-Net de difusión cada bloque emplea \textit{group
    normalization} y activaciones SiLU en lugar de \textit{batch
    normalization} y ReLU, e inyecta además el \textit{embedding} del
    paso temporal $t$ (sumándolo a los mapas de características entre
    sus dos convoluciones), de manera que el comportamiento del bloque
    queda condicionado al nivel de ruido.}
...
\begin{figure}[h!]
    ...
    autoatendidos. El coste de calcular el mapa de atención crece de
    forma \emph{cuadrática} con el número de posiciones espaciales
    $N = H \times W$, lo que obliga a reservar estos bloques para los
    niveles de menor resolución.}
...
calentamiento (\textit{warmup}). Este planificador modula la tasa de
aprendizaje en dos fases: durante los primeros 500 pasos de
entrenamiento (\textit{warmup}) la tasa crece linealmente desde 0
hasta su valor máximo $\eta$, lo que evita actualizaciones bruscas
de los pesos cuando el modelo aún está sin entrenar; a partir de ahí
la tasa decae siguiendo una curva cosenoidal —suave al principio,
más pronunciada en la zona media y de nuevo suave al final— hasta
aproximarse a 0 en la última época, de modo que el ajuste final de
los parámetros se realiza con pasos cada vez más pequeños. El
